	...
	<body>
		...
		<script>
			...
			layui.use(['paging', 'form'], function() {
                ...
                paging.init({
                    openWait: true,
					url: '/pay_channel_token/list?v=' + new Date().getTime(), //地址
					elem: '#content', //内容容器
					params: { //发送到服务端的参数
					},
					type: 'GET',
                ...
                function viewForm(id) {
                    ...
                    $.get('/pay_channel_token/view.html?id=' + id, null, function(form) {
				...
				function editForm(id) {
                    //本表单通过ajax加载 --以模板的形式，当然你也可以直接写在页面上读取
                    $.get('/pay_channel_token/edit.html?id=' + id, null, function(form) {
                        addBoxIndex = layer.open({
                            type: 1,
                            title: '保存渠道',
                            content: form,
                            ...
                            success: function(layero, index) {
                                ...
                                form.on('submit(edit)', function(data) {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "/pay_channel_token/save",
                                        data: "params=" + encodeURIComponent(JSON.stringify(data.field)),
