...
public abstract class CacheService<V>
{
  ...
  private Map<String, ZSetOperations.TypedTuple<String>> getScoreMap(final String key, V data) {
    ...
    scoreMap.forEach(new BiConsumer<String, Double>()
        {
  ...
  private AllCacheData getCacheDataMap(List<V> list) {
    if (list != null && list.size() > 0) {
      ...
      for (V t : list) {
        ...
        CacheData<V> cacheData = (new CacheData<>()).setData(t).setKey(primaryKey).setScoreMap(scoreMap).setGroupMap(groupMap).setUnionKeyMap(unionKeyMap);
  ...
  private CacheData<V> getCacheData(V data) {
    if (data != null) {
      ...
      CacheData<V> cacheData = (new CacheData<>()).setData(data).setKey(primaryKey).setScoreMap(scoreMap).setGroupMap(groupMap).setUnionKeyMap(unionKeyMap);
      return cacheData;
    }
    return null;
  }
  ...
  public void putAllToIndependentMap(List<V> dataList) {
    for (V v : dataList) {
      this.redisUtil.hPutAll(getPrimaryKey(v), BeanConvertUtils.bean2Map(v));
    }
  }
  ...
  private void putToCache(final CacheData<V> cacheData) {
    ...
    cacheData.getScoreMap().forEach(new BiConsumer<String, ZSetOperations.TypedTuple<String>>()
        {
          public void accept(final String scoreKey, final ZSetOperations.TypedTuple<String> stringTypedTuple) {
            cacheData.getGroupMap().forEach(new BiConsumer<String, String>()
                {
                  public void accept(String groupKey, String s2) {
            ...
            cacheData.getUnionKeyMap().forEach(new BiConsumer<String, String>()
                {
                  public void accept(String unionKey, String s2) {
  ...
  private void putAllToCache(Map<String, V> dataMap, Map<String, Map<String, ZSetOperations.TypedTuple<String>>> scoreMap) {
    ...
    scoreMap.forEach(new BiConsumer<String, Map<String, ZSetOperations.TypedTuple<String>>>()
        {
          public void accept(String s, Map<String, ZSetOperations.TypedTuple<String>> stringTypedTupleMap) {
            stringTypedTupleMap.forEach(new BiConsumer<String, ZSetOperations.TypedTuple<String>>()
                {
                  public void accept(String s, ZSetOperations.TypedTuple<String> stringTypedTuple) {
  ...
  private void putAllToCache(AllCacheData<V> allCacheData) {
    ...
    cacheDataMap.forEach(new BiConsumer<String, CacheData<V>>()
        {
          public void accept(String s, CacheData<V> vCacheData) {
