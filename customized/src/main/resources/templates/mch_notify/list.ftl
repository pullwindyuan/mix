	...
	<body>
		...
		<script type="text/html" id="tpl">
			...
			<tr>
				...
				<td>
					<a href="javascript:;" data-id="{{ item.orderId }}" data-opt="view" data-type="{{ item.orderType }}" class="layui-btn layui-btn-normal layui-btn-mini">查看详情</a>
					<a href="javascript:;" data-id="{{ item.orderId }}" data-opt="edit" data-type="{{ item.orderType }}" class="layui-btn layui-btn-mini">发送通知</a>
		...
		<script>
						...
						$('#content').children('tr').each(function() {
							...
							$that.children('td:last-child').children('a[data-opt=edit]').on('click', function() {
                                ...
                                layer.confirm('确定重新发送通知么？', {
									...
									sendNotify($info.data("id"), $info.data("type"));
                                }, function(){
                                });
							});
						});
            ...
            function sendNotify(id, orderType) {
                //这里可以写ajax方法提交表单
				var api = "";
				if(orderType == 1) {
				    api = "${sendNotifyPayUrl}";
				}else if(orderType == 3){
                    api = "${sendNotifyRefundUrl}";
				}
                $.ajax.base = ""
                $.ajax({
                    type: "POST",
